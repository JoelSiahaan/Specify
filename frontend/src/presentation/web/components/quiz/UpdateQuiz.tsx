/**
 * UpdateQuiz Component
 * 
 * Form for updating an existing quiz.
 * Shows editing restrictions (before due date and no submissions).
...
export const UpdateQuiz: React.FC<UpdateQuizProps> = ({
  quizId,
  onSuccess,
  onCancel,
}) => {
  ...
  const [editRestrictionMessage, setEditRestrictionMessage] = useState<string>('');
  ...
  useEffect(() => {
    const loadQuiz = async () => {
      try {
        ...
        const localDateTime = new Date(dueDateObj.getTime() - dueDateObj.getTimezoneOffset() * 60000)
          .toISOString()
          .slice(0, 16);
        ...
        // Check if quiz can be edited (before due date)
        const now = new Date();
        const isPastDueDate = dueDateObj <= now;
        
        if (isPastDueDate) {
          setCanEdit(false);
          setEditRestrictionMessage('This quiz cannot be edited because the due date has passed.');
        }
      } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!canEdit) {
      setError(editRestrictionMessage);
      return;
    }
    ...
    try {
      ...
      if (apiError.code === 'QUIZ_HAS_SUBMISSIONS') {
        setCanEdit(false);
        setEditRestrictionMessage('This quiz cannot be edited because students have already submitted answers.');
      } else if (apiError.code === 'QUIZ_PAST_DUE_DATE') {
        setCanEdit(false);
        setEditRestrictionMessage('This quiz cannot be edited because the due date has passed.');
      } else if (apiError.code === 'VALIDATION_FAILED' && apiError.details) {
        setValidationErrors(apiError.details);
      } else {
        setError(apiError.message || 'Failed to update quiz. Please try again.');
      }
    } finally {
      setLoading(false);
    }
      ...
      {!canEdit && (
        <div className="border-l-4 border-yellow-500 bg-yellow-50 p-4 mb-6">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h4 className="font-semibold text-yellow-900">Editing Restricted</h4>
              <p className="text-sm text-yellow-700">{editRestrictionMessage}</p>
            </div>
          </div>
        </div>
      )}
      ...
      <form onSubmit={handleSubmit} noValidate>
        ...
        <div className="mb-4">
          ...
          <textarea
            ...
            } ${loading || !canEdit ? 'bg-gray-100 cursor-not-allowed' : ''}`}
          />
          {validationErrors.description && (
